...
r_levels = {
    1: {"rms": 0.001, "length": 0.005, "angle": 0.25},
    2: {"rms": 0.01, "length": 0.05, "angle": 2.5},
    3: {"rms": 0.1, "length": 0.1, "angle": 5.0},
}
...
def get_complete_dataframes(energy_cutoff: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    cg_mofs, a_mofs = remove_high_energy_structures(
    ...
    cg_zeolites, a_zeolites = remove_high_energy_structures(
...
def get_all_data(struct_type: str, coarse_grain: bool) -> pd.DataFrame:
    ...
    for b in all_batches:
        ...
        if b == 1:
            ...
            structures = read(
                hZIF_data / f"{struct_type}/MOFFF/{file_id}.xyz", index=":"
            )
...
def get_reference_structure() -> Atoms:
    ...
    sorted_structures = sorted(
        ls_structures, key=lambda x: np.nansum(x.arrays["e_local_mofff"])
    )
    ref_struct = sorted_structures[0]

    error_message = (
        "The reference structure is not the ZIF zni structure. Check the database."
    )
    assert ref_struct.info["id"] == "AB2_MOF-128", error_message
